...
# Usage:
#   ./infra/docker.sh up [-d] [-p PORT] [--build]
#   ./infra/docker.sh down
#   ./infra/docker.sh restart [-d] [-p PORT]
#   ./infra/docker.sh logs [-f] [--tail N]
#   ./infra/docker.sh ps
#   ./infra/docker.sh poller
# ==============================================================================
...
case "$COMMAND" in
  up)
    ...
    if [[ -n "$DETACH" ]]; then
      ...
      echo "📜 View live logs: ./infra/docker.sh logs"
    fi
    ;;
